package com.common.tempkiosk.faceserver;
...
import com.common.tempkiosk.model.FaceRegisterInfo;
...
public class FaceServer {
     ...
     * @return 是否注册成功
     */
    public boolean registerNv21(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, int mask, String name) {
        synchronized (this) {
            ...
            //特征提取
            int code = faceEngine.extractFaceFeature(nv21, width, height, FaceEngine.CP_PAF_NV21,
                    faceInfo, ExtractType.REGISTER, MaskInfo.NOT_WORN, faceFeature);
    ...
    public boolean registerBgr24(Context context, byte[] bgr24, int width, int height, String name) {
        synchronized (this) {
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                //特征提取
                code = faceEngine.extractFaceFeature(bgr24, width, height, FaceEngine.CP_PAF_BGR24, faceInfoList.get(0), ExtractType.REGISTER, MaskInfo.NOT_WORN, faceFeature);
                String userName = name == null ? String.valueOf(System.currentTimeMillis()) : name;
                try {
                    //保存注册结果（注册图、特征数据）
                    if (code == ErrorInfo.MOK) {
